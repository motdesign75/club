...
<head>
  <meta charset="utf-8">
  <title>{{ $title ?? 'Aufgabe bearbeiten' }}</title>
...
<div class="wrap">
  ...
  <h1 style="margin:8px 0 12px">Aufgabe bearbeiten</h1>
  ...
  <form method="post" action="{{ route('projects.tasks.update', [$project, $task]) }}">
    ...
    <div class="row">
      <div>
        ...
        <input id="percent_done" name="percent_done" type="number" min="0" max="100" step="1" value="{{ old('percent_done', $task->percent_done ?? 0) }}" required>
      </div>
      <div>
        <label for="priority">Priorität (1=hoch ... 5=niedrig) *</label>
        <input id="priority" name="priority" type="number" min="1" max="5" step="1" value="{{ old('priority', $task->priority ?? 3) }}" required>
      </div>
    </div>

    <label for="type">Typ *</label>
    @php $tp = old('type', $task->type ?? 'task'); @endphp
    <select id="type" name="type" required>
      <option value="task"      {{ $tp==='task'?'selected':'' }}>Aufgabe</option>
      <option value="milestone" {{ $tp==='milestone'?'selected':'' }}>Meilenstein</option>
    </select>
    <div class="row">
      <div>
        <label for="starts_at">Startdatum</label>
        <input id="plan_start" name="plan_start" type="date" value="{{ old('plan_start', optional($task->plan_start)->toDateString()) }}">
      </div>
      <div>
        <label for="ends_at">Enddatum</label>
    ...
    <select id="status" name="status" required>
      ...
      <option value="open"        {{ $st==='open'?'selected':'' }}>Offen</option>
      <option value="in_progress" {{ $st==='in_progress'?'selected':'' }}>In Arbeit</option>
      <option value="blocked"     {{ $st==='blocked'?'selected':'' }}>Blockiert</option>
      <option value="done"        {{ $st==='done'?'selected':'' }}>Erledigt</option>
    </select>

    <div class="actions">
      <button type="submit" class="btn">💾 Speichern</button>
      <a href="{{ route('projects.show', $project) }}" class="btn secondary">Abbrechen</a>
    </div>
  </form>
</div>
</body>
